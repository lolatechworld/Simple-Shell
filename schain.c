 ...
 */
int fchain(finfo_a17 *finfo, char *buf, size_t *f)
{
	size_t j = *f;
 ...
 * Return: Void
 */
void check_fchain(finfo_a17 *finfo, char *buf, size_t *f, size_t i, size_t len)
{
	size_t j = *f;
 ...
 */
int replace_falias(finfo_a17 *finfo)
{
	int i;
	list_t *node;
	char *f;
	...
	for (i = 0; i < 10; i++)
	{
		node = fnode_starts_with((*finfo).falias, (*finfo).argv[0], '=');
		if (!node)
			return (0);
		free((*finfo).argv[0]);
		f = _strchr((*node).fstr, '=');
 ...
 * Return: 1 if replaced, 0 otherwise
 */
int replace_fvars(finfo_a17 *finfo)
{
	int i = 0;
	list_t *node;

	for (i = 0; (*finfo).argv[i]; i++)
	{
		if ((*finfo).argv[i][0] != '$' || !(*finfo).argv[i][1])
			continue;

		if (!_strcmp((*finfo).argv[i], "$?"))
		{
			replace_fstring(&((*finfo).argv[i]),
		...
		node = fnode_starts_with((*finfo).env, &(*finfo).argv[i][1], '=');
		if (node)
		{
			replace_fstring(&((*finfo).argv[i]),
		...
		replace_fstring(&(*finfo).argv[i], _strdup(""));
