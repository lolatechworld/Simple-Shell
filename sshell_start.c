 ...
 */
int shell(finfo_a17 *finfo, char **av)
{
	ssize_t r = 0;
	int builtin_ret = 0;
 ...
 */
int find_builtin(finfo_a17 *finfo)
{
	...
	for (i = 0; builtintbl[i].type; i++)
		if (_strcmp((*finfo).argv[0], builtintbl[i].type) == 0)
		{
			(*finfo).line_count++;
 ...
 * Return: void
 */
void find_cmd(finfo_a17 *finfo)
{
	char *path = NULL;
	int i, k;

	(*finfo).path = (*finfo).argv[0];
	if ((*finfo).linecount_flag == 1)
	{
		(*finfo).line_count++;
		(*finfo).linecount_flag = 0;
	}
	for (i = 0, k = 0; (*finfo).arg[i]; i++)
		if (!fdelim((*finfo).arg[i], " \t\n"))
			k++;
	if (!k)
		return;
